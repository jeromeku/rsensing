...
def crossvalidate( (Gstrn,lstrn,Gstst,lstst,L,trainalg,epochs) ):
    import auxil.supervisedclass as sc 
    if   trainalg == 1:
...
#  initialize classifier  
    if   trainalg == 1:
        ...
        classifier = sc.Ffnbp(Gs,ls,L,epochs)
    elif trainalg == 3:
        classifier = sc.Ffncg(Gs,ls,L,epochs)
    elif trainalg == 4:
...
#  cross-validation        
    ...
    for i in range(10):
        ...
        traintest.append( (np.delete(Gs,sl,0),np.delete(ls,sl,0), \
                                     Gs[sl,:],ls[sl,:],L,trainalg,epochs) )
    rc = Client()      
    v = rc[:]
    print 'submitting cross-validation to %i IPython engines'%len(rc) 
    v.block = True
    start = time.time()
    result = v.map(crossvalidate,traintest)  # block=True returns list of results
